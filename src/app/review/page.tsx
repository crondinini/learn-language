...
export default function GlobalReviewPage() {
  ...
  async function submitReview(rating: number) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // If "Again", add card to end of queue to review again this session
        if (rating === 1) {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      {/* Header */}
      <header className="flex-shrink-0 border-b border-slate-200 bg-white/80 backdrop-blur dark:border-slate-700 dark:bg-slate-900/80">
      ...
      {/* Flashcard */}
      <main className="flex flex-1 flex-col items-center justify-center overflow-y-auto p-6 pb-16">
        <div className="flex w-full max-w-2xl flex-col">
          ...
          >
            <div
              className="relative h-64 transition-transform duration-500 sm:h-72 md:h-80"
              style={{
                ...
                <div className="text-center">
                  {currentCard.image_url && (
                    <img
                      ...
                      alt=""
                      className={`mx-auto rounded-xl object-cover shadow-md ${imageOnly ? "h-36 w-36 mb-4 sm:h-40 sm:w-40" : "h-24 w-24 mb-4 sm:mb-6 sm:h-28 sm:w-28"}`}
                    />
                  )}
          ...
          {isFlipped && schedulingInfo && (
            <div className="mb-6 mt-4 grid flex-shrink-0 grid-cols-4 gap-2 sm:mt-6 sm:gap-3 md:mt-8">
              <button
                onClick={() => submitReview(1)}
                disabled={isSubmitting}
                className="group relative rounded-xl border-2 border-red-200 bg-red-50 p-2 transition hover:border-red-300 hover:bg-red-100 disabled:opacity-50 dark:border-red-900/50 dark:bg-red-900/20 dark:hover:bg-red-900/30 sm:p-3 md:p-4"
              >
                <div className="mb-1 text-lg font-bold text-red-600 sm:mb-2 sm:text-xl md:text-2xl">Again</div>
                <div className="text-xs font-medium text-red-500 sm:text-sm">
                  {formatInterval(schedulingInfo.again.card.scheduled_days)}
                </div>
                <div className="mt-1 hidden text-xs text-slate-400 sm:block">Press 1</div>
              </button>

              <button
                onClick={() => submitReview(2)}
                disabled={isSubmitting}
                className="group relative rounded-xl border-2 border-amber-200 bg-amber-50 p-2 transition hover:border-amber-300 hover:bg-amber-100 disabled:opacity-50 dark:border-amber-900/50 dark:bg-amber-900/20 dark:hover:bg-amber-900/30 sm:p-3 md:p-4"
              >
                <div className="mb-1 text-lg font-bold text-amber-600 sm:mb-2 sm:text-xl md:text-2xl">Hard</div>
                <div className="text-xs font-medium text-amber-500 sm:text-sm">
                  {formatInterval(schedulingInfo.hard.card.scheduled_days)}
                </div>
                <div className="mt-1 hidden text-xs text-slate-400 sm:block">Press 2</div>
              </button>

              <button
                onClick={() => submitReview(3)}
                disabled={isSubmitting}
                className="group relative rounded-xl border-2 border-emerald-200 bg-emerald-50 p-2 transition hover:border-emerald-300 hover:bg-emerald-100 disabled:opacity-50 dark:border-emerald-900/50 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/30 sm:p-3 md:p-4"
              >
                <div className="mb-1 text-lg font-bold text-emerald-600 sm:mb-2 sm:text-xl md:text-2xl">Good</div>
                <div className="text-xs font-medium text-emerald-500 sm:text-sm">
                  {formatInterval(schedulingInfo.good.card.scheduled_days)}
                </div>
                <div className="mt-1 hidden text-xs text-slate-400 sm:block">Press 3</div>
              </button>

              <button
                onClick={() => submitReview(4)}
                disabled={isSubmitting}
                className="group relative rounded-xl border-2 border-blue-200 bg-blue-50 p-2 transition hover:border-blue-300 hover:bg-blue-100 disabled:opacity-50 dark:border-blue-900/50 dark:bg-blue-900/20 dark:hover:bg-blue-900/30 sm:p-3 md:p-4"
              >
                <div className="mb-1 text-lg font-bold text-blue-600 sm:mb-2 sm:text-xl md:text-2xl">Easy</div>
                <div className="text-xs font-medium text-blue-500 sm:text-sm">
                  {formatInterval(schedulingInfo.easy.card.scheduled_days)}
                </div>
                <div className="mt-1 hidden text-xs text-slate-400 sm:block">Press 4</div>
              </button>
            </div>
          )}
        </div>
