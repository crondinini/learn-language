...
Build on Mac, push to Pi's local registry, backup DB, pull and restart:

```bash
# Build and push (only changed layers transfer - fast!)
...
**IMPORTANT**: Always backup the database before deploying to prevent data loss.
