...
export async function predictNextPeriod(
  ...
  // Need at least 3 completed periods to predict
  const completedPeriods = userPeriods.filter((p) => p.endDate);
  if (completedPeriods.length < 3) {
    return null;
  }

  // Use last 6 cycles (or all if fewer)
  const recent = completedPeriods.slice(-6);

  // Calculate cycle lengths (gap between consecutive start dates)
  ...
  // Use the most recent period (completed or not) as reference
