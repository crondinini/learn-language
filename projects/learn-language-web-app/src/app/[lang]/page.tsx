...
export default function Home() {
  ...
  const totalDue = filteredDecks.reduce((sum, d) => sum + d.due_cards, 0);
          ...
          <div>
            ...
            {totalDue > 0 && (
              <p className="mt-1 text-sm text-ink-soft">
                <span className="font-medium tabular-nums text-accent">{totalDue}</span> cards due across {filteredDecks.length} decks
              </p>
            )}
          </div>
          {totalDue > 0 ? (
        ...
        {isLoading ? (
          ...
          <div className="rounded-[var(--radius-md)] border-2 border-dashed border-line p-12 text-center">
            ...
            <p className="mt-1 text-sm text-ink-faint">
              Create your first deck to start learning {langConfig?.name || language}
            </p>
            <button
          ...
          <div className="stagger-children grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredDecks.map((deck) => {
              ...
              return (
                ...
                >
                  <Link href={`/${language}/deck/${deck.id}`} className="absolute inset-0 z-[1] rounded-[var(--radius-md)]" />
